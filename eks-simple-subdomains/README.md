...
│ │                            │  └─────────────────────────────────────────────────────────┘  │                            │
  ...
  │                              │                                                           │                              │ │
                                 ▼                                                           ▼
│ │ ┌─────────────────────────────────────────────────────────┐ ┌─────────────────────────────────────────────────────────┐ │
│   │                     service:whoami                      │ │                      service:echo                       │
  │ ├─────────────────────────────────────────────────────────┤ ├─────────────────────────────────────────────────────────┤ │ │
    │                                                         │ │                                                         │   │
  │ │                    deployment:whoami                    │ │                     deployment:echo                     │ │
│   │                                                         │ │                                                         │
│ │ └─────────────────────────────────────────────────────────┘ └─────────────────────────────────────────────────────────┘ │
                                                                                                                              │
  └ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ┘ │
...
1. a request reaches the ALB.
2. the ALB terminats TLS using the wildcard ACM cert.
3. the request is sent to the ingress-nginx services for which the ALB has been provisioned.
4. the ingress-nginx deployment routes the request based on the subdomain to the appropriate service. this is controlled
   with an ingress (see `src/components/httpbin/ingress.yaml`). the ingress objects effectively tell ingress-nginx to
   create a server block with the definitions required to send requests to the correct upstream.

Additional services can be registered to be served by the same ALB simply by adding an ingress for them.

NOTES:

1. we have some additional `external-dns` annotations on these specifically to disable `external-dns`. this is specific
   to the `nuon_dns` set up in the sandbox.
2. you can copy this pattern as is OR install ingress-nginx separately yourselves as its own component. the latter gives
   you more control. the former is faster.
...
| echo    | [echo.{{ .nuon.sandbox.outputs.nuon_dns.public_domain.name }}](https://echo.{{ .nuon.sandbox.outputs.nuon_dns.public_domain.name }})       |
| httpbin | [httpbin.{{ .nuon.sandbox.outputs.nuon_dns.public_domain.name }}](https://httpbin.{{ .nuon.sandbox.outputs.nuon_dns.public_domain.name }}) |

NOTE: httpbin is giving me grief
