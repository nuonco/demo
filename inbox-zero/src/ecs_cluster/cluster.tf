...
# - Fargate: for ephemeral, on-demand workloads (builder task)
# - EC2: for long-lived, steady-state workloads (App, Web services)
...
module "ecs_cluster" {
  ...
  # Fargate capacity providers for ephemeral workloads (builder)
  fargate_capacity_providers = {
    FARGATE = {
      default_capacity_provider_strategy = {
        weight = 0 # Not default, only used when explicitly specified
      }
    }
    FARGATE_SPOT = {
      default_capacity_provider_strategy = {
  ...
  autoscaling_capacity_providers = {
    ec2 = {
      ...
      default_capacity_provider_strategy = {
        weight = 100 # Default to EC2 for long-lived services
        base   = 1
      }
    }
  }
